...
(task-options!
  pom {:project     'memkits/unfolding-leaf
       :version     +version+
       :description "Unfolding Leaf"
       :url         "https://github.com/Memkits/unfolding-leaf"
       :scm         {:url "https://github.com/Memkits/unfolding-leaf"}
...
(defn html-dsl [data fileset]
  (make-html
    (let [script-name "main.js"
      ...
      (html {}
      (head {}
        (title (use-text "Unfolding Leaf"))
        (link {:attrs {:rel "icon" :type "image/png"
          :href "http://logo.cirru.org/cirru-400x400.png"}})
...
(deftask dev []
  ...
  (comp
    ...
    (watch)
    (transform-cirru)
    (reload :on-jsload 'unfolding-leaf.core/on-jsload)
    (cljs)
    (target)))
...
(deftask rsync []
  (fn [next-task]
    (fn [fileset]
      (sh "rsync" "-r" "target/"
        "tiye:repo/Memkits/unfolding-leaf" "--exclude" "main.out" "--delete")
      (next-task fileset))))
...
(deftask watch-test []
  ...
  (comp
    (watch)
    (transform-cirru)
    (test :namespaces '#{unfolding-leaf-test})))
